...
func TestLoad_jsonAttributesVariables(t *testing.T) {
	c, err := LoadFile(filepath.Join(fixtureDir, "attributes-variables.tf.json"))
	if err != nil {
		t.Fatalf("Bad: %s", err)
	}
	...
	actual := resourcesStr(c.Resources)
	print(actual)
	if actual != strings.TrimSpace(jsonAttributeVariableStr) {
		t.Fatalf("bad:\n%s", actual)
	}
...
const jsonAttributeVariableStr = `
cloudstack_firewall[test] (x1)
  ipaddress
  rule
  vars
